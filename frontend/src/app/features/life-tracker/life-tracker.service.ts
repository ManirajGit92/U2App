...
export class LifeTrackerService {
    ...
    this.mentalHealthSubject.next([
      {
        ...
        mood: 8,
        sleep: 7.5,
        reflection: 'Feeling productive and calm today.',
      },
    ]);
  ...
  private getSubjectForCategory(category: CategoryType): BehaviorSubject<any[]> {
    switch (category) {
      case 'Routines':
        return this.routinesSubject;
      case 'Expenses':
        return this.expensesSubject;
      case 'Diet':
        return this.dietSubject;
      case 'Fitness':
      ...
      case 'Investments':
        return this.investmentsSubject;
      default:
  ...
  async loadFromFirestore(): Promise<void> {
    ...
    try {
      ...
      const mentalHealth = await this.syncService.pullFromFirestore<MentalHealthEntry>(
        APP_NAME,
        'mentalHealth',
      );
      if (mentalHealth.length > 0) this.mentalHealthSubject.next(mentalHealth);

      const relationships = await this.syncService.pullFromFirestore<RelationshipEntry>(
        APP_NAME,
        'relationships',
      );
      if (relationships.length > 0) this.relationshipsSubject.next(relationships);

      const investments = await this.syncService.pullFromFirestore<InvestmentEntry>(
        APP_NAME,
        'investments',
      );
  ...
  getInsights(): string[] {
    ...
    if (totalExpenses > 500)
      insights.push(
        '⚠️ Your expenses are higher than average this week. Consider reviewing your budget.',
      );
    ...
    if (avgSleep < 7)
      insights.push(
        '😴 You averaged less than 7 hours of sleep. Prioritize rest tonight for better mental health.',
      );

    if (insights.length === 0)
      insights.push('✨ Great job! You are maintaining a balanced lifestyle.');
